...
import { BrowserRouter as Router, NavLink, Route, Routes, Navigate, useLocation } from 'react-router-dom';
import AOS from 'aos';
      ...
      <div className={`md:flex flex-grow items-center justify-center ${isOpen ? 'block' : 'hidden'}`}>
        <div className="text-white md:flex md:justify-end md:space-x-4">
          <div className="md:flex items-center justify-end space-x-4">
            ...
            <NavLink to="/" className={({ isActive }) => isActive ? 'bg-white rounded-box block px-4 py-2 text-black font-bold hover:bg-base-200' : 'block px-4 py-2 text-black font-bold hover:bg-base-200'}>Sign Out</NavLink>
          </div>
        </div>
      </div>
    </nav>
